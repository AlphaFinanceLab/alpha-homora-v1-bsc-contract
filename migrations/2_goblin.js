...
const StrategyAllBNBOnly = artifacts.require("StrategyAllBNBOnly");
...
module.exports = function (deployer, network, [creator]) {
  ...
  deployer.then(async () => {
    ...
    const wbnb = await WBNB.at(await router.WETH());
    ...
    await factory.createPair(wbnb.address, token.address);
    const pair = await factory.getPair(token.address, wbnb.address);
    const lp = await UniswapV2Pair.at(pair);

    await deployer.deploy(StrategyAllBNBOnly, router.address);
    const addStrat = await StrategyAllBNBOnly.deployed();
